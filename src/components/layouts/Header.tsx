...
import { ShoppingCart, User, Search, Menu, X, Phone, MapPin, ChevronDown, Heart, LogOut, Settings, Package, LayoutDashboard, Globe, Sun, Moon, Scale } from 'lucide-react';
      ...
      <div className="bg-secondary border-b border-border hidden md:block">
        <div className="max-w-7xl mx-auto px-4 py-1.5 flex items-center justify-between text-xs text-muted-foreground">
          <div className="flex items-center gap-6">
            <a href={`tel:+${settings.whatsapp_number || '994776117780'}`} className="flex items-center gap-1.5 hover:text-foreground transition-colors">
              <Phone size={11} className="text-primary" />+{settings.whatsapp_number || '994776117780'}
            </a>
                  ...
                  <DropdownMenuContent align="end" className="bg-secondary border-border w-52">
                    ...
                    {(isAdmin || isMod) && (
                      <>
                        ...
                        <DropdownMenuItem asChild>
                          <Link to={isAdmin ? "/admin" : "/admin/products"} className="cursor-pointer text-primary"><LayoutDashboard size={13} className="mr-2" />{t('admin_panel')}</Link>
                        </DropdownMenuItem>
                      </>
                    )}
