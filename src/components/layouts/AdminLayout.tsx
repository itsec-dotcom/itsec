...
import { LayoutDashboard, Package, ShoppingCart, Users, FileText, Settings, Image, Star, LogOut, Globe, BarChart2, Palette } from 'lucide-react';
...
const ADMIN_NAV = [
  ...
  { label: 'Kompaniya Dəstləri', icon: Package, href: '/admin/promo-kits' },
  ...
  { label: 'Tools/Downloads', icon: FileText, href: '/admin/tools' },
  { label: 'Theme Settings', icon: Palette, href: '/admin/theme' },
...
export const AdminLayout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const visibleNav = ADMIN_NAV.filter(item => {
    ...
    if (item.href === '/admin/products' || item.href === '/admin/categories') return true;
    return false;
  });
      ...
      <aside className="hidden md:flex flex-col w-60 shrink-0 bg-secondary border-r border-border">
        <div className="p-4 border-b border-border flex items-center justify-between">
          <div>
            <Link to="/" className="flex items-center">
              {settings.site_logo ? (
                <img src={settings.site_logo} alt={settings.site_title} className="h-6 object-contain" />
              ) : (
                <span className="text-lg font-black text-foreground">{settings.site_title || 'itsec.az'}</span>
              )}
            </Link>
        ...
        <div className="px-4 py-3 border-b border-border bg-primary/5">
          ...
          <div className="flex items-center gap-1.5 mt-0.5">
            ...
            <span className="text-xs text-green-400 font-medium">{isAdmin ? 'Administrator' : 'Köməkçi'}</span>
          </div>
        </div>
        ...
        <div className="p-3 border-t border-border">
          ...
          <div className="mt-3 pt-3 border-t border-border/50 text-center">
            <p className="text-[10px] text-muted-foreground/60">Powered by <span className="text-primary font-semibold">ITSEC</span></p>
          </div>
        </div>
      </aside>
