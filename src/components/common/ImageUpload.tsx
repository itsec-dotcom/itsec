/**
 * ImageUpload — Reusable image uploader for itsec.az
...
async function compressToWebP(file: File, bucketType: string): Promise<{ blob: Blob; wasCompressed: boolean; finalSize: number }> {
  // Banners need higher resolution, use 2560. Others can use 1080.
  const MAX_DIM = bucketType === 'banners' ? 2560 : 1200;
  const QUALITY = bucketType === 'banners' ? 0.9 : 0.8;
...
interface ImageUploadProps {
  bucket: 'products' | 'banners' | 'avatars' | 'blog' | 'categories';
  path?: string;
  currentUrl?: string;
...
const ImageUpload: React.FC<ImageUploadProps> = ({
  ...
  const handleFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      // Compress if needed
      const { blob, wasCompressed, finalSize } = await compressToWebP(file, bucket);
      setProgress(50);
