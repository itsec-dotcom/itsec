...
import { Filter, Grid, List, ShoppingCart, Heart, Search, ChevronLeft, ChevronRight, Package, Scale } from 'lucide-react';
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="flex flex-wrap gap-2 mb-5">
        {[
          { label: `📷 ${t('cat_all_cameras')}`, value: '' },
          { label: `🏠 ${t('cat_indoor')}`, value: 'indoor-cameras' },
          { label: `🌧 ${t('cat_outdoor')}`, value: 'outdoor-cameras' },
          { label: `📡 ${t('cat_ip')}`, value: 'ip-cameras' },
                    ...
                    <div className="p-4">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex gap-1">
                          <Button size="icon" variant="ghost" className={`h-8 w-8 ${isInCompare(p.id) ? 'text-primary bg-primary/10' : 'text-muted-foreground hover:text-primary'}`} onClick={() => isInCompare(p.id) ? removeFromCompare(p.id) : addToCompare(p)}><Scale size={14} /></Button>
