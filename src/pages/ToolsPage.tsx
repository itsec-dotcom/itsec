...
const SoftwareDownloads: React.FC = () => {
  ...
  let systems = [];
  try {
    systems = JSON.parse(settings.software_downloads || '[]');
  } catch (err) {
    systems = [];
  }

  const getIcon = (name: string) => {
    if (name === 'Monitor') return <Monitor size={14} />;
    if (name === 'Smartphone') return <Smartphone size={14} />;
    ...
    <div className="space-y-4">
      <div className="mb-2">
        <h3 className="text-base font-bold text-foreground flex items-center gap-2 mb-1">
          <Download size={18} className="text-primary" /> {t('tools_software_title')}
        </h3>
        <p className="text-xs text-muted-foreground">{t('tools_software_subtitle')}</p>
      </div>
      {systems.map((sys: any) => (
        <Card key={sys.brand} className={`bg-card border p-4 ${sys.bgColor}`}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {sys.items.map((item: any, i: number) => (
              <a
                key={i}
                href={item.url}
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center justify-between p-2 rounded bg-background border border-border/50 hover:border-primary/50 hover:shadow-sm transition-all group"
              >
                <div className="flex items-center gap-2 text-xs font-medium text-foreground">
                  <div className={`p-1.5 rounded-full ${sys.bgColor} ${sys.color}`}>
                    {getIcon(item.icon)}
                  </div>
                  {item.label}
                </div>
...
const ToolsPage: React.FC = () => {
  ...
  const TABS = [
    { id: 'downloads', label: t('tools_tab_downloads'), icon: Download },
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-foreground mb-1">{t('tools_title')}</h1>
        <p className="text-muted-foreground text-sm">{t('tools_subtitle')}</p>
      </div>
      ...
      <div className="mt-8 bg-card border border-border rounded p-6 text-center">
        ...
        <a href="https://wa.me/994776117780" target="_blank" rel="noopener noreferrer">
          <Button className="bg-green-700 hover:bg-green-600 text-white">{t('tools_consult')}</Button>
        </a>
      </div>
    </div>
  );
